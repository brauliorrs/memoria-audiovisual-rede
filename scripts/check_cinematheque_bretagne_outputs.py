...
from memoria_audiovisual.output_files import CINEMATHEQUE_BRETAGNE_OUTPUT_FILES
...
def main():
    ...
    missing_files = [
        CINEMATHEQUE_BRETAGNE_OUTPUT_FILES[key]
        for key in required_keys
        if not (OUTPUT_DIR / CINEMATHEQUE_BRETAGNE_OUTPUT_FILES[key]).exists()
    ]

    summary_df = load_csv_if_exists(CINEMATHEQUE_BRETAGNE_OUTPUT_FILES["summary"])
    if summary_df is None or summary_df.empty:
        print("O arquivo principal da Cinémathèque de Bretagne está ausente ou vazio.")
        return 1

    links_df = load_csv_if_exists(CINEMATHEQUE_BRETAGNE_OUTPUT_FILES["video_links"])
    analytic_summary_df = load_csv_if_exists(CINEMATHEQUE_BRETAGNE_OUTPUT_FILES["analytic_summary"])
    analytic_video_catalog_df = load_csv_if_exists(CINEMATHEQUE_BRETAGNE_OUTPUT_FILES["analytic_video_catalog"])
    ...
    print("Validação do corpus Cinémathèque de Bretagne")
    ...
    if missing_files:
        ...
        print("- todos os arquivos esperados da Cinémathèque de Bretagne foram encontrados")

    snapshot_metadata = load_json_if_exists(CINEMATHEQUE_BRETAGNE_OUTPUT_FILES["snapshot_metadata"])
    if snapshot_metadata:
        print("- metadados da rodada:")
        print(f"  - fonte Cinémathèque de Bretagne: {snapshot_metadata.get('source_url', '-')}")
