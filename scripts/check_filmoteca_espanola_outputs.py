...
from memoria_audiovisual.filmoteca_espanola import FILMOTECA_ESPANOLA_MIN_VIDEO_TOTAL
from memoria_audiovisual.output_files import FILMOTECA_ESPANOLA_OUTPUT_FILES
...
def main():
    ...
    missing_files = [
        FILMOTECA_ESPANOLA_OUTPUT_FILES[key]
        for key in required_keys
        if not (OUTPUT_DIR / FILMOTECA_ESPANOLA_OUTPUT_FILES[key]).exists()
    ]
    summary_df = load_csv_if_exists(FILMOTECA_ESPANOLA_OUTPUT_FILES["summary"])
    links_df = load_csv_if_exists(FILMOTECA_ESPANOLA_OUTPUT_FILES["video_links"])
    catalog_df = load_csv_if_exists(FILMOTECA_ESPANOLA_OUTPUT_FILES["analytic_video_catalog"])
    ...
    print("Validação do corpus Filmoteca Española / PLATFO FILMO")
    print(f"- instituições na base: {0 if summary_df is None else len(summary_df)}")
    print(f"- registros PLATFO com titularidade Filmoteca Española: {len(links_df)}")
    ...
    snapshot_metadata = load_json_if_exists(FILMOTECA_ESPANOLA_OUTPUT_FILES["snapshot_metadata"])
    if snapshot_metadata:
        ...
        print(f"- arquivo de metadados ausente: {FILMOTECA_ESPANOLA_OUTPUT_FILES['snapshot_metadata']}")

    if summary_df is None or summary_df.empty:
        print("- resumo principal ausente ou vazio")
        return 1
    if len(links_df) < FILMOTECA_ESPANOLA_MIN_VIDEO_TOTAL:
        print(f"- total abaixo do piso metodológico: {len(links_df)} de {FILMOTECA_ESPANOLA_MIN_VIDEO_TOTAL}")
        return 1
    ...
    print("- todos os arquivos esperados da Filmoteca Española / PLATFO FILMO foram encontrados")
    return 0
