# Pós-flight da revisão periódica Estado–tecnologia

## Finalidade

O pós-flight é a barreira entre uma coleta aparentemente concluída e a consolidação da rodada na branch `statetech-history`. Ele não corrige dados nem altera relatórios: apenas verifica se os produtos da rodada são semanticamente coerentes.
...
```text
data/statetech/postflight/<snapshot_id>.json
```
