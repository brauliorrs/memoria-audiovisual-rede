...
```text
scripts/preflight_digital_infrastructure_periodic_review.py
```
...
```text
data/digital_infrastructure/preflight/<snapshot_id>.json
```

Somente um relatório com `ok: true` permite avançar para a auditoria. O relatório também é preservado na branch `digital-infrastructure-history` e incluído no artefato operacional temporário.
