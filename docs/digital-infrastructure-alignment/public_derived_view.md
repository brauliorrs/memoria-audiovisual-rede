# Visão pública derivada infraestrutura digital
...
```text
data/digital_infrastructure/public/
...
```bash
python scripts/build_digital_infrastructure_public_view.py \
...
O workflow periódico gera essa visão depois da triagem e antes da consolidação na branch `digital-infrastructure-history`.

## Limite deliberado

A existência de um item em `data/digital_infrastructure/public` significa apenas que ele é elegível para uma camada pública futura. Não há implantação automática em site, API, dashboard ou rede social.
