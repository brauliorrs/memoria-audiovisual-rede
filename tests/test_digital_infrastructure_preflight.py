...
from memoria_audiovisual.digital_infrastructure.preflight import PeriodicReviewPreflight


class PeriodicReviewPreflightTests(unittest.TestCase):
    def _repository(self, root: Path) -> None:
        (root / "schemas/digital_infrastructure").mkdir(parents=True)
        ...
        schema = root / "schemas/digital_infrastructure/example.schema.json"
        schema.write_text("{}", encoding="utf-8")
        registry = {"schema_version": "1", "schemas": [{"entity": "example", "path": "schemas/digital_infrastructure/example.schema.json"}]}
        (root / "schemas/digital_infrastructure/schema_registry.json").write_text(json.dumps(registry), encoding="utf-8")

    def test_clean_first_run_is_accepted(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self._repository(root)
            report = PeriodicReviewPreflight(root, root / "data/digital_infrastructure").validate(
