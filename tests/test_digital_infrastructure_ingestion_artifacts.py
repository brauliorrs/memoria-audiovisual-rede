...
from memoria_audiovisual.digital_infrastructure.ingestion import (
    IngestionCoordinator,
    IngestionResumeMismatch,
)
from memoria_audiovisual.digital_infrastructure.ingestion_batches import (
    BatchManifestStore,
)
from memoria_audiovisual.digital_infrastructure.locking import (
    FileWriteLock,
    LedgerLockTimeout,
)
from memoria_audiovisual.digital_infrastructure.models import (
    EntityRecord,
    ProvenanceRecord,
)
...
def _record(key: str, *, value: str | None = None) -> AdaptedRecord:
    return AdaptedRecord(
        entity_type="digital_infrastructure_audit",
        natural_key=key,
        payload={"observation_id": key, "value": value or key},
...
class IngestionArtifactTests(unittest.TestCase):
    def test_same_source_produces_same_artifact_batch_and_fingerprint(self) -> None:
    ...
    def test_resume_aborts_before_persistence_when_adapted_output_changes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            service = _Service(fail_on=2)
            coordinator = IngestionCoordinator(
                service,  # type: ignore[arg-type]
            ...
            with self.assertRaises(RuntimeError):
                coordinator.commit(
                    _Adapter((_record("a"), _record("b"))),
                    source,
                )
            self.assertEqual(len(service.registered), 1)

            with self.assertRaises(IngestionResumeMismatch):
                coordinator.commit(
                    _Adapter((_record("a", value="changed"), _record("b"))),
                    source,
                )
            self.assertEqual(len(service.registered), 1)

    def test_batch_lock_blocks_parallel_commit_for_same_batch(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "batches.jsonl"
            first = BatchManifestStore(path, lock_timeout=0.05)
            second = BatchManifestStore(path, lock_timeout=0.05)
            batch_id = "same-batch"

            with first.batch_lock(batch_id):
                with self.assertRaises(LedgerLockTimeout):
                    with second.batch_lock(batch_id):
                        pass

    def test_manifest_append_uses_file_lock(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "batches.jsonl"
            store = BatchManifestStore(path, lock_timeout=0.05)
            manifest = store.latest("missing")
            self.assertIsNone(manifest)

            with FileWriteLock(path, timeout=0.05):
                from memoria_audiovisual.digital_infrastructure.ingestion_batches import (
                    BatchManifest,
                )
