...
from memoria_audiovisual.statetech.postflight import validate_periodic_run
...
class PostflightTests(unittest.TestCase):
    ...
    def test_valid_run_passes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            self._prepare(root)
            report = validate_periodic_run(snapshot_id="snapshot_test", state_dir=root / "data" / "statetech")
