"""Regras executáveis de integridade para o núcleo de infraestrutura digital."""
        ...
        for entry in ledger.read_all():
            for envelope in entry.records:
                ...
                if record_type == "entity_version":
                    ...
                    version = str(payload["version_id"])
                    entities.add(entity_id)
                    versions.add(version)
                    latest[entity_id] = version
                elif record_type == "evidence":
                    evidences.add(str(payload["evidence_id"]))
        return cls(frozenset(entities), frozenset(versions), frozenset(evidences), latest)
...
class IntegrityValidator:
    ...
    def validate_entity_version(self, *, entity_id: str, version_id: str, previous_version_id: str | None) -> None:
        ...
        if previous_version_id != latest:
            raise IntegrityError(f"cadeia de versões inválida: esperado {latest}, recebido {previous_version_id}")
    ...
    def validate_entity_references(self, referenced_entity_ids: tuple[str, ...], *, pending_entity_ids: tuple[str, ...] = ()) -> None:
        index = LedgerIndex.build(self.ledger)
    ...
    def validate_evidence_references(self, evidence_ids: tuple[str, ...], *, pending_evidence_ids: tuple[str, ...] = ()) -> None:
        index = LedgerIndex.build(self.ledger)
