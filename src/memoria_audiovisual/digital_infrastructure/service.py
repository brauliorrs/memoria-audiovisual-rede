...
class StatetechDataService:
    """Nome histórico preservado durante a migração de namespace."""
    ...
    def register_entity(self, *, entity_type: str, natural_key: str, payload: dict[str, Any], provenance: ProvenanceRecord, evidences: tuple[EvidenceRecord, ...] = (), previous_version_id: str | None = None, referenced_entity_ids: tuple[str, ...] = (), validation_status: ValidationStatus = "pending_review") -> EntityRecord:
        ...
        entity = EntityRecord(entity_type=entity_type, entity_id=entity_id, payload=schema_record, validation_status=validation_status, previous_version_id=previous_version_id)
        ...
        self.integrity.validate_entity_version(entity_id=entity_id, version_id=entity.version_id, previous_version_id=previous_version_id)
        self.integrity.validate_evidence_ids(evidence_ids)
        self.integrity.validate_entity_references(referenced_entity_ids)
        self.integrity.validate_evidence_references(linked_evidence_ids, pending_evidence_ids=evidence_ids)
    ...
    def register_entity_decision(self, decision: EntityDecision) -> dict[str, Any]:
        ...
        referenced_entities = tuple(dict.fromkeys((*decision.source_entity_ids, *decision.target_entity_ids)))
        missing_entities = sorted(set(referenced_entities) - set(index.entities))
        if missing_entities:
            raise IntegrityError(f"decisão referencia entidades inexistentes: {', '.join(missing_entities)}")
        ...
        if decision.supersedes_decision_id is not None and not any(item.decision_id == decision.supersedes_decision_id for item in existing_decisions):
            raise IntegrityError(f"decisão substituída inexistente: {decision.supersedes_decision_id}")
    ...
    def _entity_decisions(self) -> tuple[EntityDecision, ...]:
        ...
        for entry in self.ledger.read_all():
            for envelope in entry.records:
                ...
                decisions.append(EntityDecision(
...
DigitalInfrastructureDataService = StatetechDataService
