"""Catálogo explicativo e verificável dos indicadores analíticos."""
...
class IndicatorCatalog:
    ...
    def load(cls, path: str | Path) -> "IndicatorCatalog":
        ...
        if not source.exists():
            raise FileNotFoundError(f"catálogo de indicadores inexistente: {source}")
        payload = json.loads(source.read_text(encoding="utf-8"))
        if not isinstance(payload, Mapping):
            raise ValueError("catálogo deve conter um objeto JSON")
        version = str(payload.get("catalog_version") or "").strip()
        if not version:
            raise ValueError("catalog_version é obrigatório")
        raw_entries = payload.get("indicators")
        if not isinstance(raw_entries, list) or not raw_entries:
            raise ValueError("catálogo deve conter uma lista não vazia de indicadores")
        entries: list[Mapping[str, Any]] = []
        seen: set[tuple[str, str]] = set()
        for position, raw in enumerate(raw_entries, start=1):
            if not isinstance(raw, Mapping):
                raise ValueError(f"entrada inválida no catálogo: posição {position}")
            missing = [field for field in _REQUIRED_TEXT_FIELDS if not str(raw.get(field) or "").strip()]
            if missing:
                raise ValueError(
            ...
            if key in seen:
                raise ValueError(f"entrada duplicada no catálogo: {key[0]}@{key[1]}")
            seen.add(key)
    ...
    def validate_registry(self, registry: IndicatorRegistry) -> None:
        ...
        registry_keys = {(indicator.indicator_id, indicator.version) for indicator in registry}
        ...
        if missing:
            ...
            raise ValueError(f"indicadores registrados sem explicação no catálogo: {formatted}")
        if orphaned:
            formatted = ", ".join(f"{item[0]}@{item[1]}" for item in orphaned)
            raise ValueError(f"entradas do catálogo sem indicador registrado: {formatted}")
